    ...
    $('.delete').click(function(){
        ...
        m.each(function(i){
            if($(this).attr('data-type')==0){
                arr_fid.push($(this).attr('data-mid'));
            }else if($(this).attr('data-type')==1){
                arr_lid.push($(this).attr('data-mid'));
            }
        });
        if(m.length>0&&(confirm('您确定要删除启事吗？'))){
            var l_ids=arr_lid.join(',');
            var f_ids=arr_fid.join(',');
            $.ajax({
                type:"post",
                url:"ajaxApi/delete.php?action=l",
                data:{l_ids:l_ids,f_ids:f_ids},
                dataType:'json',
