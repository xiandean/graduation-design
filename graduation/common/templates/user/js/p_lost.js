            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.error==0){
                        ...
                        if(data.count==0){
                            alert('发布成功!系统暂未匹配到相关记录！'); 
                            if($type.val()==0){
                                window.location.href="detail.php?type=found&l_id="+data.l_id;
                            }else if($type.val()==1){
                                window.location.href="detail.php?type=lost&l_id="+data.l_id;
                            }
                            
                        }else if(data.count>0&&confirm("发布成功!系统共匹配到"+data.count+"条相识记录，是否要查看？")){
                            window.location.href="ucenter.php?p=list&action=pulling";
                        }
...
singleCheck("answer1_wrap");
// multiCheck("field_wrap");
